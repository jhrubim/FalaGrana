...
export default function ContasCartoesScreen() {
...
const salvarConta = async () => {
  ...
  try {
    const { data: authData, error: authError } = await supabase.auth.getUser();
    if (authError || !authData?.user?.id) {
      throw new Error('Usuário não autenticado para salvar a conta.');
    }
    ...
    const payload: any = {
      ...
      usuario_id: userId, // força aderência à policy
      nome,
      tipo: form.tipo,
      ativa: form.ativa,
    ...
    const instituicao = form.instituicao.trim();
    if (instituicao) {
      payload.instituicao = instituicao;
    } else {
    ...
    if (form.id) {
      ...
      const { error } = await supabase
        .from('contas')
        .insert([payload]); // em array evita incompatibilidade em alguns cenários web
    ...
    Alert.alert('Erro ao salvar conta', msg || 'Falha ao salvar.');
  } finally {
    setSalvando(false);
  }
};
  ...
  const alternarStatusConta = async (conta: Conta) => {
    ...
    const mensagem = `Deseja ${conta.ativa ? 'inativar' : 'reativar'} a conta "${conta.nome}"?`;
            ...
            {contasFiltradas.map((conta) => (
              ...
              >
                ...
                <View style={styles.listItemRight}>
                  ...
                  {!isViewer ? (
                    <Pressable onPress={() => alternarStatusConta(conta)} style={styles.actionLink}>
            ...
            <View style={styles.modalActions}>
              ...
              >
                {salvando ? (
                  ...
                  <Text style={styles.primaryButtonText}>{form.id ? 'Salvar' : 'Criar conta'}</Text>
                )}
              </Pressable>
            </View>
          </View>
